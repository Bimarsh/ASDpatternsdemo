...
public class ShippingBillingWindow extends JDialog implements ParentWindow {
	...
	private final String ADDRESS = "Address 1";
	...
	class ProceedCheckoutButtonListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent evt) {
        	...
        	java.util.List<String> updates = null;
        	if(isNewShipAddress()) {
        		 
        		try {
        			//set up
        			String moduleName = "rules-address";
        			File rulesFile =new File("address-rules.clp");//new File("address-rules-enc.txt");
        			
        			String deftemplateName = "address-template";
        			AddressBean addrbean = new AddressBean(getShipAddressFields());
        			...
        			rulesOk=false;
        			setVisible(true);
        			displayError(ShippingBillingWindow.this,e.getMessage());
        		}	
        	}
        	if(rulesOk) {
